...
enum EntityType   { PLAYER, BLOCK, WIND, NONE };
...
public:
    ...
    void moveUp()    { mMovement.y = -0.3; mDirection = UP;    }
    void moveDown()  { mMovement.y =  0.3; mDirection = DOWN;  }
    void moveLeft()  { mMovement.x = -0.3; mDirection = LEFT;  }
    void moveRight() { mMovement.x =  0.3; mDirection = RIGHT; }

    void accelLeft()  { mAcceleration.x -= 40.0f; mDirection = LEFT;  }
    void accelRight() { mAcceleration.x += 40.0f; mDirection = RIGHT; }
    void driftStop() {
        if (mDirection == RIGHT) {
            if (mAcceleration.x > 0.0f) mAcceleration.x -= 150.0f;
            else                        mAcceleration.x  = 0.0f;
        }
